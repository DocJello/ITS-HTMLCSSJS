...
function TeacherDashboard({ user, token }: { user: User, token: string }) {
  ...
  const createAssessment = async (e: FormEvent) => {
    ...
    // Feature Enforcement: Formative assessments must have 4-5 easy questions only.
    if (newAssessment.type === 'formative') {
      if (newAssessment.questionIds.length < 4 || newAssessment.questionIds.length > 5) {
        return alert("Formative assessments must have exactly 4 or 5 questions.");
      }
      // Check if they are 'easy'
      const selectedQs = allQuestions.filter(q => newAssessment.questionIds.includes((q as any)._id || q.id));
      const nonEasy = selectedQs.filter(q => q.difficulty !== DifficultyLevel.EASY);
      if (nonEasy.length > 0) {
        return alert("Formative assessments must only contain EASY questions.");
      }
    }

    // Feature Enforcement: Summative assessments must have 10 questions.
    if (newAssessment.type === 'summative') {
       if (newAssessment.questionIds.length !== 10) {
         return alert("Summative assessments must have exactly 10 questions.");
       }
    }
...
function StudentModules({ user, token, onStartAssessment }: { user: User; token: string; onStartAssessment: (topic: string, type: 'formative' | 'summative', subId: string, customAssessment?: Assessment) => void }) {
  ...
  return (
    <div className="max-w-[1128px] mx-auto px-4 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
      <div className="space-y-6">
        {allSummativesPassed && !selectedSubmodule && (
          <div className="bg-gradient-to-r from-linkedin-blue to-blue-800 p-8 rounded-xl text-white shadow-xl mb-6">
             <div className="flex items-center gap-4 mb-4">
               <Sparkles size={32} className="text-yellow-300" />
               <h2 className="text-2xl font-black uppercase tracking-tighter">Congratulations!</h2>
             </div>
             <p className="font-bold text-blue-100 mb-6 leading-relaxed">
               You have successfully completed all core modules. To finalize your certification, please complete the Final Exam. 
               Note: This post-test uses the same assessment framework as your initial pre-test to measure your growth.
             </p>
             <button 
               onClick={() => onStartAssessment("Course Baseline", "summative", "course-posttest")}
               className="bg-white text-linkedin-blue px-8 py-3 rounded-full font-black uppercase tracking-widest text-xs shadow-lg hover:scale-105 active:scale-95 transition-all"
             >
               Start Final Exam (Post-test)
             </button>
          </div>
        )}
        {selectedSubmodule ? (
...
function AdminDashboard({ user, token }: { user: User; token: string }) {
  ...
  const [loading, setLoading] = useState(true);
  const [adminView, setAdminView] = useState<"sections" | "students">("sections");
  const [showSectionModal, setShowSectionModal] = useState(false);
  const [newSection, setNewSection] = useState({ name: "", teacherId: "" });

  const fetchData = async () => {
    try {
      const [secRes, teachRes, studRes] = await Promise.all([
        fetch("/api/sections", { headers: { Authorization: `Bearer ${token}` } }),
    ...
    <div className="max-w-[1128px] mx-auto px-4 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md">
            ...
            <form onSubmit={handleCreateSection} className="space-y-4">
              ...
              <select 
                ...
                required
              >
                <option value="">Select Initial Teacher</option>
                {teachers.map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
              </select>
              <div className="flex gap-4 pt-2">
                <button type="submit" className="flex-1 bg-linkedin-blue text-white py-2 rounded-full font-bold text-sm">Create</button>
                <button type="button" onClick={() => setShowSectionModal(false)} className="px-6 py-2 border rounded-full font-bold text-sm text-gray-400">Cancel</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      <div className="bg-white p-8 rounded-xl border border-linkedin-border shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-linkedin-text-muted font-bold tracking-tight">System-wide resource management</p>
            </div>
          </div>
          <div className="flex gap-2 bg-gray-100 p-1 rounded-full">
            <button 
              onClick={() => setAdminView("sections")}
              className={`px-6 py-1.5 rounded-full text-xs font-bold transition-all ${adminView === 'sections' ? 'bg-white shadow text-linkedin-blue' : 'text-gray-500 hover:text-linkedin-text'}`}
            >
              Sections
            </button>
            <button 
              onClick={() => setAdminView("students")}
              className={`px-6 py-1.5 rounded-full text-xs font-bold transition-all ${adminView === 'students' ? 'bg-white shadow text-linkedin-blue' : 'text-gray-500 hover:text-linkedin-text'}`}
            >
              Students
            </button>
          </div>
        ...
        {adminView === "sections" ? (
          <div className="bg-white rounded-lg border border-linkedin-border overflow-hidden">
            <div className="px-6 py-4 border-b border-linkedin-border bg-gray-50 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-[10px] font-black uppercase tracking-widest py-1.5 px-4 bg-linkedin-blue text-white rounded-full shadow-sm"
              >
                + New Section
              </button>
            </div>
          ...
          <div className="bg-white rounded-lg border border-linkedin-border overflow-hidden">
            <div className="px-6 py-4 border-b border-linkedin-border bg-gray-50 flex items-center justify-between">
              <h3 className="text-xs font-black uppercase tracking-widest text-linkedin-text-muted">
                Manage Students ({students.length})
              </h3>
              {selectedStudents.length > 0 && (
                <button 
                  onClick={bulkDelete}
                  className="bg-red-500 text-white px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest shadow-md flex items-center gap-2"
                >
                  <Trash2 size={12} /> Bulk Delete ({selectedStudents.length})
                </button>
              )}
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-linkedin-border text-[10px] font-black uppercase tracking-wider text-gray-400">
                    ...
                    <th className="px-6 py-4">Pre-test</th>
                    <th className="px-6 py-4">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {students.map((stud) => (
                    <tr key={stud._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedStudents.includes(stud._id)}
                          onChange={() => setSelectedStudents(prev => prev.includes(stud._id) ? prev.filter(i => i !== stud._id) : [...prev, stud._id])}
                        />
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-xs">{stud.sectionId?.name || "Unassigned"}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${stud.preTestCompleted ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                          {stud.preTestCompleted ? 'Done' : 'Pending'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <button onClick={() => deleteStudent(stud._id)} className="text-red-500 hover:text-red-700 p-2"><Trash2 size={16} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function App() {
  ...
  const [appState, setAppState] = useState<"solving" | "reflecting" | "complete" | "teacher_dashboard" | "admin_dashboard" | "modules" | "pre_test">("modules");
  ...
  const startCoursePreTest = async () => {
    try {
      const res = await fetch("/api/assessments/course-baseline", {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      setExercises(data);
  ...
  const startAssessment = async (topic: string, type: 'formative' | 'summative', subId: string, customAssessment?: Assessment) => {
    try {
      ...
      if (subId === 'course-posttest' || subId === 'course-pretest') {
        const res = await fetch("/api/assessments/course-baseline", {
           headers: { Authorization: `Bearer ${token}` }
        });
        filtered = await res.json();
      } else if (customAssessment && customAssessment.questionIds) {
  ...
  const handleNext = () => {
    ...
    if (currentIdx < exercises.length - 1) {
      ...
      setAppState("complete");
      if (assessmentMode) {
        if (assessmentMode.id === 'course-pretest') {
          const finishPreTest = async () => {
             const res = await fetch("/api/auth/complete-pre-test", {
               method: "POST",
               headers: { Authorization: `Bearer ${token}` }
             });
             const data = await res.json();
          ...
          finishPreTest();
          alert(`Initial Pre-test completed! Score: ${assessmentScore}/${assessmentTotalPoints}. You can now begin your modules.`);
        } else {
